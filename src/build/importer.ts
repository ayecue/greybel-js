...
export const executeImport = async (
  ...
  if (successfulItems.length === 0) {
    ...
    logger.debug(`Import was only partially successful. Only ${successfulItems.length} files got imported to ${options.ingameDirectory}!`);
    return false;
  }

  logger.debug(`${successfulItems.length} files got imported to ${options.ingameDirectory}!`);

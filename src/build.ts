...
function createInstaller(parseResult: TranspilerParseResult, mainTarget: string, buildPath: string, maxWords: number): void {
	...
	const createInstallerFile = function() {
        ...
        const target = path.resolve(buildPath, 'installer' + installerSplits + '.src');
...
export default async function build(filepath: string, output: string, options: BuildOptions = {}): Promise<boolean> {
	...
	try {
		...
		await Promise.all(Object.entries(result).map(async ([file, code]) => {
